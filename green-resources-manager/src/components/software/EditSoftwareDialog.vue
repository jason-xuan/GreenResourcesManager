    ...
    <div class="modal-wrapper">
      ...
      <div class="modal-body">
        ...
        <FormField 
          ...
          placeholder="选择软件可执行文件"
          @browse="handleBrowseExecutable" 
        />
        <!-- 封面图片选择区域 -->
        <div class="form-group">
          <label class="form-label">软件封面</label>
  ...
  methods: {
    loadSoftwareData() {
      ...
      this.formData = {
        ...
        developer: this.software.developer || '',
        description: this.software.description || '',
        tags: Array.isArray(this.software.tags) ? [...this.software.tags] : [],
        executablePath: this.software.executablePath || '',
        imagePath: this.software.image || ''
      }
      this.tagInput = ''
    },
    handleClose() {
    ...
    async handleUseScreenshotAsCover() {
      try {
        if (!this.formData.name) {
          await alertService.warning('请先输入软件名称', '提示')
          return
        }

        if (!this.formData.id) {
          await alertService.warning('软件ID不存在，无法打开截图文件', '提示')
          return
        }
    ...
    handleConfirm() {
      const updatedSoftware = {
        id: this.formData.id,
        name: this.formData.name.trim() || this.software.name,
        developer: (this.formData.developer || '').trim(),
        description: (this.formData.description || '').trim(),
        tags: [...this.formData.tags],
        executablePath: this.formData.executablePath.trim() || this.software.executablePath,
        image: (this.formData.imagePath || '').trim()
      }

      this.$emit('confirm', updatedSoftware)
    },
